...
void* computeSum(void* args) { // pass in the number of the ith thread
    ...
    int *iteration = (int *)args;

    for(int i=0;i <MAX; i++){
      matSumResult[*iteration][i] = matA[*iteration][i] + matB[*iteration][i];
    }
    pthread_exit(NULL);
}
...
void* computeDiff(void* args) { // pass in the number of the ith thread
    ...
    for(int i=0;i <MAX; i++){
      matDiffResult[*iteration][i] = matA[*iteration][i] - matB[*iteration][i];
    }
    pthread_exit(NULL);
...
void* computeProduct(void* args) { // pass in the number of the ith thread
    ...
    for(int i=0;i <MAX; i++){
      for (int j =0; j < MAX; j++) {
        matProductResult[i][j] = matProductResult[i][j] + (matA[i][*iteration]*matB[*iteration][j]);
      }
    }
    pthread_exit(NULL);
}
...
int main() {
    ...
    for (int i = 0; i < MAX; i++) {
    pthread_t tid;
    pthread_create(&tid, NULL, computeSum, (void *)&i);
    ...
    for (int i = 0; i < MAX; i++) {
    pthread_t tid;
    pthread_create(&tid, NULL, computeDiff, (void *)&i);
    pthread_join(tid, NULL);
    }
    ...
    for (int i = 0; i < MAX; i++) {
    pthread_t tid;
    pthread_create(&tid, NULL, computeProduct, (void *)&i);
    pthread_join(tid, NULL);
    }
